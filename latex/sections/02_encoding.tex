...
\begin{equation}
x = (-1)^{p_0} \cdot 2^{E - \text{bias}} \cdot \left(1 + M\right)
\label{eq:decode}
\end{equation}
where $E = \sum_{i=1}^{n_e} p_i \cdot 2^{n_e-i}$ is the exponent and $M = \sum_{j=1}^{n_m} p_{n_e+j} \cdot 2^{-j}$ is the mantissa fraction.
