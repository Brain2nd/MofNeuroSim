...
The RMSNorm backward requires careful handling of the Jacobian. Since rms depends on $\mathbf{x}$, the gradient includes a correction term. Let $r = \text{rms}(\mathbf{x})^{-1}$. The complete backward formulas are:

\begin{align}
\frac{\partial \mathcal{L}}{\partial \gamma_i} &= \sum_{\text{batch}} \frac{\partial \mathcal{L}}{\partial y_i} \cdot \hat{x}_i \nonumber \\
\frac{\partial \mathcal{L}}{\partial x_i} &= \frac{\partial \mathcal{L}}{\partial y_i} \cdot \gamma_i \cdot r - x_i \cdot r^3 \cdot \frac{1}{n} \sum_j \frac{\partial \mathcal{L}}{\partial y_j} \gamma_j x_j
\label{eq:rmsnorm_backward}
\end{align}
where $\hat{x}_i = x_i \cdot r$ denotes the normalized input. The second term in $\frac{\partial \mathcal{L}}{\partial x_i}$ is the Jacobian correction accounting for the dependence of $r$ on $\mathbf{x}$.
